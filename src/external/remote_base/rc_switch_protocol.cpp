...
const RCSwitchBase RC_SWITCH_PROTOCOLS[10] = {RCSwitchBase(0, 0, 0, 0, 0, 0, false),
                                             RCSwitchBase(770, 790, 260, 480, 480, 260, false),
...
bool RCSwitchBase::expect_sync(RemoteReceiveData &src) const {
    ...
    int max_sync_pulses = (src.size() > 6) ? 6 : src.size();
    for (int i = 0; i < max_sync_pulses; i++) {
        // trying up to max_sync_pulses times we need at least one successful sync  
