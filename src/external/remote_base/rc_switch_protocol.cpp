...
const RCSwitchBase RC_SWITCH_PROTOCOLS[10] = {RCSwitchBase(0, 0, 0, 0, 0, 0, false),
                                             RCSwitchBase(773, 803, 228, 511, 468, 266, false),
...
bool RCSwitchBase::expect_sync_single(RemoteReceiveData &src) const {
  if (!this->inverted_) {
...
bool RCSwitchBase::expect_sync(RemoteReceiveData &src) const {
    ...
    for (int i = 0; i < src.size(); i++) {
        // trying up to nearly end of presented data we need at least one successful sync  
        // If after at least one successful sync follows a non successful sync we exit the loop and do not advance further
        if (this->expect_sync_single(src))
            result++;        // cumulate successful syncs
        else
                if (result)
                    break;           // stop after trailing non successful sync
                else
                    src.advance(2);  // step over leading non successful sync
            }
    return result;
  }
...
optional<RCSwitchData> RCSwitchBase::decode(RemoteReceiveData &src) const {
  ...
  for (uint8_t i = 1; i <= 8; i++) {
    ...
    if (protocol->decode(src, &out.code, &out_nbits) && out_nbits >= 3) {
      ...
      // helper code to identify reasons of decoding breaks
      if ((out.protocol == 1) && (out.size < 41)) {
        RawTimings data = src.get_raw_data();
        int offset      = src.get_index(); // at this point decoding failed
        ESP_LOGI(TAG, "Failed here: protocol:%u, len: %d, offset: %d, data: %d %d %d %d * %d %d %d %d", 
            out.protocol, src.size(), offset,
